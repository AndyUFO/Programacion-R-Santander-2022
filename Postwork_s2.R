...
ggplot(iris.full, aes(x=Sepal.Length , y=Sepal.Width , color=Species, size= Petal.Width))+geom_point(shape=10, alpha=0.5)
...
iris_mean<- iris.full %>%
            group_by(Species) %>%
            summarize(Sepal.lenght.mean=mean(Sepal.Length),
...
gfull<-ggplot(iris.full, aes(x=Sepal.Length , y=Sepal.Width , color=Species, size= Petal.Width))+geom_point(shape=10, alpha=0.5)
gfull
gfull <- gfull + ggplot(iris_mean, aes(x=Sepal.lenght.mean , y=Sepal.width.mean))+geom_point()
